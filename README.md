...
- следует активировать купон AWS, полученный от координатора курса;
- бюджет купона ограничен, это нужно иметь в виду при проектировании инфраструктуры и использовании ресурсов;
...
1. Настройте [workspaces](https://www.terraform.io/docs/language/state/workspaces.html).
   ...
   1. Альтернативный вариант: используйте один workspace, назвав его *stage*. Пожалуйста, не используйте workspace, создаваемый Terraform по умолчанию (*default*).
...
1. В случае использования [Terraform Cloud](https://app.terraform.io/) в качестве [backend](https://www.terraform.io/docs/language/settings/backends/index.html) убедитесь, что применение изменений успешно проходит, используя веб-интерфейс Terraform cloud.
...
1. Альтернативый вариант: воспользуйтесь сервисом [EKS](https://aws.amazon.com/eks/) (Amazon Elastic Kubernetes Service).
   1. Воспользуйтесь модулем [terraform-aws-eks](https://github.com/terraform-aws-modules/terraform-aws-eks).
...
Рекомендуемые способы выполнения:
...
1. Если на первом этапе вы не воспользовались [Terraform Cloud](https://app.terraform.io/), тогда задеплойте в кластер [atlantis](https://www.runatlantis.io/) для отслеживания изменений инфраструктуры.
...
2. Http доступ к веб-интерфейсу grafana.
...
2. Пример pull request с комментариями, созданными atlantis или снимки экрана из Terraform Cloud.
