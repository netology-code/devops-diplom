...
Цель:
1. Автоматическая сборка docker образа и загрузка его в регистр докер (docker registry) при коммите в репозиторий с тестовым приложением.
2. Автоматический деплой нового docker образа при создании нового тега в репозитории с приложением.

Можно использовать [teamcity](https://www.jetbrains.com/ru-ru/teamcity/), [GitLab](https://gitlab.com/) или [jenkins](https://www.jenkins.io/)

Ожидаемый результат:
1. Teamcity или jenkins доступны по http. В случае использования GitLab, проект находится в открытом доступе.
2. При любом коммите в git-репозиторий с тестовым приложением происходит сборка и отправка в регистр образов (docker registry) образа.
3. При создании тега в git-репозитории происходит сборка docker-образа с тегом, соответсвующим созданному git-тегу, загрузка в регистр образов (docker registry) и последующий запуск собранного образа в kubernetes-кластере..
Конфигурация CI должна обеспечивать следующий процесс:
1. Разработчик вносит изменения в код приложения и отправляет commit в репозиторий.
2. CI-сервер собрает docker-образ с приложением, используя код, полученный на предыдущем шаге, готовый docker-образ с уникальным тегом (например, можно использовать commit hash) загружается в docker registry. На этом автоматизация заканчивается, позволяя разработчику, например,  протестировать собранный образ локально.
3. Разработчик устанавливает новый git-тег с желаемым номером версии приложения (например, `0.0.1`) и отправляет этот тег в репозиторий.
4. По событию появления нового git-тега CI-сервер запускает сборку docker-образа из кода, на который указывает появившийся тег. Происходит также версионирование собранного образа тегом, определяющим версию (например, тем же тегом, который был установлен в git) и последующая загрузка в docker registry.
5. После того как образ с тегом, полученный на предыдущем шаге запускается в kubernetes-кластере. На этом автоматизированный процесс заканчивается.
